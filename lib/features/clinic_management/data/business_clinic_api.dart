...
class BusinessClinicApi {
  ...
  Future<Paginated<BusinessClinicAppointment>> appointments({
    String? status,
    int page = 1,
  }) async {
    final data =
  ...
  Future<BusinessClinicAppointment> reschedule(
    int id,
    ...
    final data =
        await _client.post(
            ...
            as Map<String, dynamic>;
    return BusinessClinicAppointment.fromJson(
  ...
  Future<BusinessClinicAppointment> _act(String path) async {
    final data =
        await _client.post('/business/clinic-appointments/$path')
            as Map<String, dynamic>;
    return BusinessClinicAppointment.fromJson(
      data['appointment'] as Map<String, dynamic>,
    );
  ...
  Future<Paginated<BusinessClinicSlot>> slots({
  ...
  Future<({int created, int skipped})> publishSlots({
    DateTime? startsAt,
    List<DateTime>? slots,
  }) async {
    final data =
  ...
  Future<void> deleteSlot(int slotId) =>
      _client.delete('/business/clinic-slots/$slotId');
}
