...
class BusinessListScreen extends ConsumerStatefulWidget {
  ...
  const BusinessListScreen({super.key, required this.childId, required this.title});
...
class _BusinessListScreenState extends ConsumerState<BusinessListScreen> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 200) {
      ref.read(businessListControllerProvider(widget.childId).notifier).loadMore();
    }
  }
  ...
  void _onSearchChanged(String value) {
    ...
    _debounce = Timer(const Duration(milliseconds: 400), () {
      ref.read(businessListControllerProvider(widget.childId).notifier).search(value);
    });
  }
          ...
          Expanded(
            child: Builder(builder: (context) {
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      OutlinedButton(
                        onPressed: () => ref.read(businessListControllerProvider(widget.childId).notifier).load(),
              ...
              return RefreshIndicator(
                onRefresh: () => ref.read(businessListControllerProvider(widget.childId).notifier).load(),
